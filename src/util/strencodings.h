...
// ============================================================================
// WATTx EVM 0x Address Utilities
// ============================================================================

/**
 * @brief Format an EVM/contract address with 0x prefix
 * @param hexAddr 20-byte hex address without prefix
 * @return Address with 0x prefix (e.g., "0x1234...")
 */
inline std::string FormatEvmAddress(const std::string& hexAddr) {
    return "0x" + hexAddr;
}

/**
 * @brief Format raw bytes as EVM address with 0x prefix
 * @param addr 20-byte address data
 * @return Address with 0x prefix
 */
inline std::string FormatEvmAddress(const std::vector<unsigned char>& addr) {
    return "0x" + HexStr(addr);
}

/**
 * @brief Strip 0x prefix from EVM address if present
 * @param addr Address string (with or without 0x prefix)
 * @return Address without 0x prefix
 */
inline std::string StripEvmAddressPrefix(const std::string& addr) {
    if (addr.size() >= 2 && (addr.substr(0, 2) == "0x" || addr.substr(0, 2) == "0X")) {
...
/**
 * @brief Validate and normalize an EVM address
 * Accepts addresses with or without 0x prefix
 * @param addr Input address string
 * @param normalized Output normalized address (lowercase, no prefix)
 * @return true if valid 20-byte hex address
 */
inline bool NormalizeEvmAddress(const std::string& addr, std::string& normalized) {
    normalized = StripEvmAddressPrefix(addr);
    // EVM address must be exactly 40 hex chars (20 bytes)
    if (normalized.size() != 40) return false;
    // Validate hex (HexDigit returns -1 for invalid characters)
