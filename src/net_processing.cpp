...
bool PeerManagerImpl::CheckPoSHeadersAreContinuous(const std::vector<CBlockHeader>& headers, const CBlockIndex* chain_start_header) const
{
    ...
    for (const CBlockHeader& header : headers)
    {
        ...
        // No rejection of PoW headers - miners and stakers compete equally
                                     ...
                                     const std::atomic<bool>& interruptMsgProc)
{
    ...
    // WATTx: Handle heartbeat messages from validators
    if (msg_type == NetMsgType::HEARTBEAT) {
        trust::Heartbeat heartbeat;
        vRecv >> heartbeat;

        // Process through heartbeat manager if available
        if (trust::g_heartbeat_manager) {
            if (trust::g_heartbeat_manager->ProcessHeartbeat(heartbeat, pfrom.GetId())) {
                LogDebug(BCLog::NET, "Received valid heartbeat from validator %s via peer=%d\n",
                         heartbeat.validatorId.ToString(), pfrom.GetId());
            }
        }
        return;
    }

    if (msg_type == NetMsgType::GETVALIDATORS) {
        // Return list of known validators
        if (trust::g_heartbeat_manager) {
            trust::ValidatorList list = trust::g_heartbeat_manager->GetValidatorList();
            MakeAndPushMessage(pfrom, NetMsgType::VALIDATORS, list);
            LogDebug(BCLog::NET, "Sent validator list (%d validators) to peer=%d\n",
                     list.validators.size(), pfrom.GetId());
        }
        return;
    }

    if (msg_type == NetMsgType::VALIDATORS) {
        trust::ValidatorList list;
        vRecv >> list;

        if (trust::g_heartbeat_manager) {
            trust::g_heartbeat_manager->ProcessValidatorList(list);
            LogDebug(BCLog::NET, "Received validator list (%d validators) from peer=%d\n",
                     list.validators.size(), pfrom.GetId());
        }
        return;
    }

    if (msg_type == NetMsgType::REGVALIDATOR) {
        trust::ValidatorRegistration reg;
        vRecv >> reg;

        if (trust::g_heartbeat_manager) {
            if (trust::g_heartbeat_manager->ProcessValidatorRegistration(reg, pfrom.GetId())) {
                LogDebug(BCLog::NET, "Processed validator registration from peer=%d\n", pfrom.GetId());
            }
        }
        return;
    }

    // WATTx: Handle encrypted P2P messages
    if (msg_type == NetMsgType::ENCMSG) {
        auto* msgMgr = messaging::g_message_manager.get();
        if (!msgMgr) return;

        try {
            messaging::EncryptedMessage msg;
            vRecv >> msg;

            if (!msg.IsValid()) return;

            if (msg.msgHash.IsNull()) {
                msg.msgHash = msg.GetHash();
            }

            msgMgr = messaging::g_message_manager.get();
            if (msgMgr) {
                msgMgr->ProcessMessage(msg, pfrom.GetId());
            }
        } catch (...) {
...
bool PeerManagerImpl::SendMessages(CNode* pto)
{
    ...
    // Relay encrypted P2P messages
    {
        auto* msgMgr = messaging::g_message_manager.get();
        if (msgMgr) {
            try {
                auto messages = msgMgr->GetMessagesToRelay(pto->GetId(), 5);
                for (const auto& msg : messages) {
                    if (msg.IsValid() && !msg.msgHash.IsNull()) {
                        MakeAndPushMessage(*pto, NetMsgType::ENCMSG, msg);
                    }
                }
            } catch (...) {
                // Silently ignore messaging errors
