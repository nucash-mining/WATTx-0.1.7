...
public:
                               ...
                               CTransactionRef coinbase, std::shared_ptr<CAuxPow> auxpow_proof) override
    {
        ...
        block.nTime = timestamp;
        block.nBits = base_block.nBits;
        block.nNonce = nonce;
        block.hashStateRoot = base_block.hashStateRoot;
        block.hashUTXORoot = base_block.hashUTXORoot;
        block.prevoutStake = base_block.prevoutStake;
        block.vchBlockSigDlgt = base_block.vchBlockSigDlgt;
        block.nShift = base_block.nShift;
        ...
        block.auxpow = auxpow_proof;

        LogPrintf("MiningInterface: Submitting AuxPoW block, hash: %s, auxpow: %s\n",
                  block.GetHash().GetHex().substr(0, 16),
                  block.auxpow ? "attached" : "MISSING");

        // Create shared_ptr - the auxpow pointer is preserved in the CBlock
        auto block_ptr = std::make_shared<const CBlock>(block);

        // ProcessNewBlock will call CheckBlock which validates AuxPoW
        return chainman().ProcessNewBlock(block_ptr, /*force_processing=*/true, /*min_pow_checked=*/true, /*new_block=*/nullptr);
    }
