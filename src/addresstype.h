...
/**
 * Dilithium (ML-DSA-65) quantum-resistant public key hash.
 * Uses HASH160 of the 1952-byte Dilithium public key.
 * Address prefix: 'Q' for mainnet, 'q' for testnet (Quantum-safe)
 */
struct DilithiumPubKeyHash : public BaseHash<uint160>
{
    DilithiumPubKeyHash() : BaseHash() {}
    explicit DilithiumPubKeyHash(const uint160& hash) : BaseHash(hash) {}
    // Constructor from raw Dilithium public key bytes
    explicit DilithiumPubKeyHash(const std::vector<unsigned char>& pubkey);
};
 ...
 */
using CTxDestination = std::variant<CNoDestination, PubKeyDestination, PKHash, ScriptHash, WitnessV0ScriptHash, WitnessV0KeyHash, WitnessV1Taproot, PayToAnchor, WitnessUnknown, DilithiumPubKeyHash>;
...
enum addresstype
{
    ...
    PAYTOANCHOR = 6,
    NONSTANDARD = 7,
    DILITHIUMPUBKEYHASH = 8  // Quantum-resistant Dilithium address
};
...
struct DataVisitor
{
    ...
    valtype operator()(const DilithiumPubKeyHash& dilithiumKeyHash) const;
};
