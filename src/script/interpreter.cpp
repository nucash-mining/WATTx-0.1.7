    ...
    try
    {
        for (; pc < pend; ++opcode_pos) {
            ...
            switch (opcode)
            {
                ...
                case OP_CHECKSIG_DILITHIUM:
                case OP_CHECKSIGVERIFY_DILITHIUM:
                {
                    // (sig pubkey -- bool)
                    // Verify a Dilithium (ML-DSA-65) post-quantum signature
                    if (stack.size() < 2)
                        return set_error(serror, SCRIPT_ERR_INVALID_STACK_OPERATION);
                    ...
                    if (vchPubKey.size() != dilithium::PUBLIC_KEY_SIZE) {
                        popstack(stack);
                        popstack(stack);
                        stack.push_back(vchFalse);
                        if (opcode == OP_CHECKSIGVERIFY_DILITHIUM)
                            return set_error(serror, SCRIPT_ERR_CHECKSIGVERIFY);
                        break;
                    }

                    if (vchSig.size() != dilithium::SIGNATURE_SIZE && !vchSig.empty()) {
                        popstack(stack);
                        popstack(stack);
                        stack.push_back(vchFalse);
                        if (opcode == OP_CHECKSIGVERIFY_DILITHIUM)
                    ...
                    // Empty signature means failure
                    bool fSuccess = false;
                    if (!vchSig.empty()) {
                        // For Dilithium signatures, we use the hash of the script code
                        // being executed as the message to verify. This is a simplified
                        // approach for post-quantum signature verification.
                        // Note: Full transaction signature hashing will be implemented
                        // when TransactionSignatureChecker is extended for Dilithium.
                        CScript scriptCode(pbegincodehash, pend);
                        uint256 scriptHash = Hash(scriptCode);

                        // Verify using Dilithium
                        dilithium::CPubKey pubkey(vchPubKey);
                        fSuccess = pubkey.Verify(scriptHash, vchSig);
                    }
