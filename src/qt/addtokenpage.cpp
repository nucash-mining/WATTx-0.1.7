    ...
    m_clientModel(0)
{
    ...
    connect(ui->lineEditDecimals, &QLineEdit::textChanged, this, &AddTokenPage::on_updateConfirmButton);
...
void AddTokenPage::on_updateConfirmButton()
{
    ...
    // Check contract address is valid (40 hex chars)
    QString contractAddress = ui->lineEditContractAddress->text();
    if(contractAddress.isEmpty() || contractAddress.length() != 40)
    {
        enabled = false;
    }
    else
    {
        ...
        QRegularExpression hexRegex("^[0-9a-fA-F]{40}$");
        if(!hexRegex.match(contractAddress).hasMatch())
        {
            enabled = false;
        }
    }
    ...
    if(ui->lineEditDecimals->text().isEmpty())
    {
        enabled = false;
    }
    else
    {
        // Verify decimals is a valid number (0-77)
        bool ok;
        int decimals = ui->lineEditDecimals->text().toInt(&ok);
        if(!ok || decimals < 0 || decimals > 77)
        {
            enabled = false;
        }
    }
