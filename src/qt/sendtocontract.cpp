...
void SendToContract::setModel(WalletModel *_model)
{
    ...
    if (bCreateUnsigned) {
        ...
        ui->pushButtonSendToContract->setToolTip(tr("Creates a Partially Signed WATTx Transaction (PSBT) for use with e.g. an offline %1 wallet, or a PSBT-compatible hardware wallet.").arg(CLIENT_NAME));
    }
}
...
void SendToContract::on_sendToContractClicked()
{
    if(isDataValid())
    {
        ...
        if (bCreateUnsigned) {
            ...
            questionString.append(tr("Please, review your transaction proposal. This will produce a Partially Signed WATTx Transaction (PSBT) which you can copy and then sign with e.g. an offline %1 wallet, or a PSBT-compatible hardware wallet.").arg(CLIENT_NAME));
