...
int GuiMain(int argc, char* argv[])
{
    ...
    // Network Selection Dialog - show at startup if no network specified on command line
    bool networkSpecified = false;
    bool useTestnet = false;
    for (int i = 1; i < argc; i++) {
        QString arg(argv[i]);
        if (arg == "-testnet" || arg == "--testnet" || arg == "-testnet=1") {
            networkSpecified = true;
            useTestnet = true;
        } else if (arg == "-mainnet" || arg == "--mainnet" || arg == "-testnet=0") {
            networkSpecified = true;
            useTestnet = false;
        }
    }

    // Show network selection dialog if not specified on command line
    if (!networkSpecified) {
        NetworkSelectionDialog netDialog;
        if (netDialog.exec() == QDialog::Accepted) {
            useTestnet = netDialog.isTestnetSelected();
        } else {
            // User cancelled - exit
            return EXIT_SUCCESS;
        }
    ...
    // Build new argument list with network selection
    std::vector<char*> newArgv;
    for (int i = 0; i < argc; i++) {
        newArgv.push_back(argv[i]);
    }
    static char testnetArg[] = "-testnet";
    if (useTestnet && !networkSpecified) {
        newArgv.push_back(testnetArg);
    }
