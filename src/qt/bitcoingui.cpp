...
void BitcoinGUI::createActions()
{
    ...
    sendCoinsAction->setStatusTip(tr("Send coins to a WATTx address"));
    ...
    miningAction->setStatusTip(tr("Mine WATTx using X25X multi-algorithm mining"));
    delegationAction = new QAction(tr("Delegations"), this);
    superStakerAction = new QAction(tr("Super Staking"), this);
    messagingAction = new QAction(platformStyle->MultiStatesIcon(":/icons/messaging"), tr("Messaging"), this);
    messagingAction->setStatusTip(tr("Send messages on-chain or chat with other wallet addresses"));
    ...
    signMessageAction->setStatusTip(tr("Sign messages with your WATTx addresses to prove you own them"));
    evmExportAction = new QAction(tr("&Export EVM Address/Key…"), this);
    evmExportAction->setStatusTip(tr("Export your address and private key for use with Ethereum wallets like Rabby or MetaMask"));
    verifyMessageAction = new QAction(tr("&Verify message…"), this);
    verifyMessageAction->setStatusTip(tr("Verify messages to ensure they were signed with specified WATTx addresses"));
    m_load_psbt_action = new QAction(tr("&Load PSBT from file…"), this);
    m_load_psbt_action->setStatusTip(tr("Load Partially Signed WATTx Transaction"));
    m_load_psbt_clipboard_action = new QAction(tr("Load PSBT from &clipboard…"), this);
    m_load_psbt_clipboard_action->setStatusTip(tr("Load Partially Signed WATTx Transaction from clipboard"));
    ...
    showHelpMessageAction->setStatusTip(tr("Show the %1 help message to get a list with possible WATTx command-line options").arg(CLIENT_NAME));
    ...
    if(walletFrame)
    {
        ...
        connect(evmExportAction, &QAction::triggered, this, &BitcoinGUI::showEvmExportDialog);
...
void BitcoinGUI::createMenuBar()
{
    ...
    if(walletFrame)
    {
        ...
        file->addSeparator();
        file->addAction(evmExportAction);
    }
...
void BitcoinGUI::createToolBars()
{
    if(walletFrame)
    {
        ...
        walletStakeActions.append(miningAction);
...
void BitcoinGUI::setClientModel(ClientModel *_clientModel, interfaces::BlockAndHeaderTipInfo* tip_info)
{
    ...
    if(_clientModel)
    {
        ...
        if(_clientModel->getOptionsModel()->getCheckForUpdates() && qtumVersionChecker->newVersionAvailable())
        {
            ...
            QString message(tr("New version of WATTx wallet is available on the WATTx source code repository: <br /> %1. <br />It is recommended to download it and update this application").arg(link));
...
void BitcoinGUI::showEvmExportDialog()
{
    if (!walletFrame) return;
    WalletModel* walletModel = walletFrame->currentWalletModel();
    if (!walletModel) return;

    EvmExportDialog dlg(this);
...
void BitcoinGUI::updateNetworkState()
{
    ...
    if (m_node.getNetworkActive()) {
        ...
        tooltip = tr("%n active connection(s) to WATTx network.", "", count);
    } else {
        //: A substring of the tooltip.
