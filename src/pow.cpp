...
/**
 * X25X Multi-Algorithm PoW Verification
 *
 * After the X25X activation height, blocks can use any of the supported
 * algorithms (SHA256d, Scrypt, Ethash, RandomX, Equihash, X11).
 * Each algorithm maintains its own difficulty chain.
 */
bool CheckProofOfWorkX25X(const CBlockHeader& header, unsigned int nBits, const Consensus::Params& params)
{
    // Get the algorithm from block version
    x25x::Algorithm algo = x25x::GetBlockAlgorithm(header.nVersion);

    // Check if algorithm is enabled
    if (!x25x::IsAlgorithmEnabled(algo)) {
        LogPrintf("CheckProofOfWorkX25X: Block uses disabled algorithm %d\n", static_cast<int>(algo));
        return false;
    }

    // Use X25X verification
    return x25x::CheckProofOfWork(header, nBits, params);
}

/**
 * Get next work required for X25X multi-algorithm mining.
 * Each algorithm has independent difficulty adjustment.
 */
unsigned int GetNextWorkRequiredX25X(const CBlockIndex* pindexLast, const CBlockHeader* pblock,
                                      x25x::Algorithm algo, const Consensus::Params& params)
{
    if (pindexLast == nullptr) {
        return UintToArith256(params.powLimit).GetCompact();
    }

    // Use X25X per-algorithm difficulty calculation
    return x25x::GetNextWorkRequiredForAlgorithm(pindexLast, algo, params);
}

/**
 * Get the mining algorithm name for logging/RPC
 */
std::string GetAlgorithmName(const CBlockHeader& header)
{
    x25x::Algorithm algo = x25x::GetBlockAlgorithm(header.nVersion);
    return x25x::GetAlgorithmInfo(algo).name;
}
