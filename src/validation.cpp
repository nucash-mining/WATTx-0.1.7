...
// Gapcoin PoW removed - replaced with X25X multi-algorithm mining
...
bool MemPoolAccept::PreChecks(ATMPArgs& args, Workspace& ws)
{
    ...
    // WATTx FCMP: Validate FCMP transaction structure (context-free)
    if (privacy::IsFcmpStateAvailable() && privacy::GetFcmpState().IsInitialized()) {
        if (!privacy::GetFcmpState().CheckFcmpTransaction(tx, state)) {
            return false; // state filled in by CheckFcmpTransaction
        }
    }

    // WATTx Privacy: Validate privacy transaction structure (context-free)
    if (privacy::IsPrivacyActive(m_active_chainstate.m_chain.Height() + 1, chainparams.GetConsensus()) &&
        privacy::HasPrivacyData(tx)) {
        auto privTx = privacy::ExtractPrivacyTransaction(tx);
        if (privTx.has_value()) {
            int nSpendHeight = m_active_chainstate.m_chain.Height() + 1;
            if (!privacy::CheckPrivacyTransaction(*privTx, state, nSpendHeight)) {
                return false; // state filled in by CheckPrivacyTransaction
            }
        }
    }
    ...
    // WATTx FCMP: Validate FCMP inputs with full context (key images, proofs)
    if (privacy::IsFcmpStateAvailable() && privacy::GetFcmpState().IsInitialized()) {
        int nSpendHeight = m_active_chainstate.m_chain.Height() + 1;
        if (!privacy::GetFcmpState().CheckFcmpInputs(tx, state, m_view, nSpendHeight)) {
            return false; // state filled in by CheckFcmpInputs
        }
    }

    // WATTx Privacy: Validate key images not already spent
    if (privacy::IsPrivacyActive(m_active_chainstate.m_chain.Height() + 1, chainparams.GetConsensus()) &&
        privacy::HasPrivacyData(tx)) {
        auto privTx = privacy::ExtractPrivacyTransaction(tx);
        if (privTx.has_value()) {
            auto keyImageDB = privacy::GetKeyImageDB();
            if (keyImageDB) {
                int nSpendHeight = m_active_chainstate.m_chain.Height() + 1;
                if (!privacy::ContextualCheckPrivacyTransaction(*privTx, *keyImageDB, state, nSpendHeight)) {
                    return false; // state filled in by ContextualCheckPrivacyTransaction
                }
            }
        }
    }
...
{
    // Without height info, use RandomX (base algorithm for Fluorine Fermie)
    // This is used during initial header sync before we have block heights
    return CheckProofOfWorkRandomX(block, block.nBits, consensusParams);
}

/**
 * Check proof of work with height-awareness for X25X activation
 * @param block The block header to validate
 * @param nHeight The height of this block (prev height + 1)
 * @param consensusParams Consensus parameters
 * @return true if PoW is valid
 */
bool CheckHeaderPoWAtHeight(const CBlockHeader& block, int nHeight, const Consensus::Params& consensusParams)
{
    // Genesis block always uses SHA256d
    if (block.GetHash() == consensusParams.hashGenesisBlock) {
        return CheckProofOfWorkImpl(block.GetHash(), block.nBits, consensusParams);
    }

    // Check if this is an AuxPoW (merged-mined) block
    // For header-only validation, we can't verify AuxPoW without the proof
    // The full proof is validated in CheckBlock when we have the complete block
    if (block.nVersion & AUXPOW_VERSION_FLAG) {
        // Check if AuxPoW is active at this height
        if (!consensus::IsAuxPowActive(nHeight, consensusParams)) {
            LogPrintf("CheckHeaderPoWAtHeight: AuxPoW flag set but not active at height %d\n", nHeight);
            return false;
        }
        // Header-only AuxPoW validation passes - full validation in CheckBlock
        // Just verify the target is valid
        arith_uint256 target;
        bool fNegative, fOverflow;
        target.SetCompact(block.nBits, &fNegative, &fOverflow);
        if (fNegative || fOverflow || target == 0) {
            LogPrintf("CheckHeaderPoWAtHeight: AuxPoW block has invalid nBits target\n");
            return false;
        }
        return true;
    }
    ...
    if (consensusParams.IsX25XActive(nHeight)) {
        ...
        // Pre-X25X: use RandomX validation (base algorithm for Fluorine Fermie)
        return CheckProofOfWorkRandomX(block, block.nBits, consensusParams);
    }
}
...
bool CheckIndexProof(const CBlockIndex& block, const Consensus::Params& consensusParams)
{
    ...
    if(block.IsProofOfStake()){
        ...
        // For PoW blocks, use height-aware validation (X25X after activation)
        CBlockHeader header = block.GetBlockHeader();
        return CheckHeaderPoWAtHeight(header, block.nHeight, consensusParams);
    }
}
...
CAmount GetBlockSubsidy(int nHeight, const Consensus::Params& consensusParams)
{
    ...
    // WATTx: Use base block reward from consensus params
    // Default: 500000000 satoshis = 5 WTX per block (50 WTX per 10 min at 2-min blocks)
    CAmount nBaseReward = consensusParams.nBaseBlockReward;

    // Calculate halvings based on height
...
DisconnectResult Chainstate::DisconnectBlock(const CBlock& block, const CBlockIndex* pindex, CCoinsViewCache& view, bool* pfClean)
{
    ...
    // WATTx: Unmark coinstake UTXO when disconnecting a PoS block (reorg)
    if (block.IsProofOfStake() && block.vtx.size() > 1 && block.vtx[1]->IsCoinStake()) {
        const CTransaction& coinstakeTx = *block.vtx[1];
        if (!coinstakeTx.vin.empty()) {
            GetCoinstakeTracker().UnmarkUTXO(coinstakeTx.vin[0].prevout, pindex->nHeight);
        }
    }
    ...
    // WATTx FCMP: Revert curve tree and key image changes for reorg
    if (privacy::IsFcmpStateAvailable() && privacy::GetFcmpState().IsInitialized()) {
    ...
    // WATTx Privacy: Remove key images for reorg
    auto keyImageDB = privacy::GetKeyImageDB();
    if (keyImageDB) {
        for (const auto& ptx : block.vtx) {
            const CTransaction& tx = *ptx;
            if (privacy::HasPrivacyData(tx)) {
                auto privTx = privacy::ExtractPrivacyTransaction(tx);
                if (privTx.has_value()) {
                    if (!privacy::DisconnectPrivacyTransaction(*privTx, *keyImageDB)) {
                        LogPrintf("Privacy: Failed to disconnect key images for tx %s\n", tx.GetHash().ToString());
                    }
                }
            }
        }
                               ...
                               CCoinsViewCache& view, bool fJustCheck)
{
    ...
    for (unsigned int i = 0; i < block.vtx.size(); i++)
    {
        ...
        if (!tx.IsCoinBase())
        {
            ...
            // WATTx Privacy: Validate privacy transaction in block
            if (privacy::IsPrivacyActive(pindex->nHeight, params.GetConsensus()) &&
                privacy::HasPrivacyData(tx)) {
                auto privTx = privacy::ExtractPrivacyTransaction(tx);
                if (privTx.has_value()) {
                    auto keyImageDB = privacy::GetKeyImageDB();
                    if (keyImageDB) {
                        TxValidationState tx_state;
                        if (!privacy::ContextualCheckPrivacyTransaction(*privTx, *keyImageDB, tx_state, pindex->nHeight)) {
                            state.Invalid(BlockValidationResult::BLOCK_CONSENSUS,
                                          tx_state.GetRejectReason(),
...
//////////////////////////////////////////////////////////////////
    ...
    // WATTx: Track coinstake UTXO to prevent double-spending
    // Mark the coinstake input as used at this height
    if (block.IsProofOfStake() && block.vtx.size() > 1 && block.vtx[1]->IsCoinStake()) {
        const CTransaction& coinstakeTx = *block.vtx[1];
        if (!coinstakeTx.vin.empty()) {
            GetCoinstakeTracker().MarkUTXOUsed(coinstakeTx.vin[0].prevout, pindex->nHeight);
        }
    }

    // WATTx FCMP: Update curve tree and key image database
    // This adds FCMP outputs to the tree and marks key images as spent
    if (privacy::IsFcmpActive(pindex->nHeight, params.GetConsensus()) &&
    ...
    // WATTx Privacy: Register spent key images
    if (privacy::IsPrivacyActive(pindex->nHeight, params.GetConsensus())) {
        auto keyImageDB = privacy::GetKeyImageDB();
        if (keyImageDB) {
            for (size_t i = 0; i < block.vtx.size(); i++) {
                const CTransaction& tx = *block.vtx[i];
                if (!tx.IsCoinBase() && privacy::HasPrivacyData(tx)) {
                    auto privTx = privacy::ExtractPrivacyTransaction(tx);
                    if (privTx.has_value()) {
                        if (!privacy::ConnectPrivacyTransaction(*privTx, *keyImageDB, tx.GetHash(), pindex->nHeight)) {
                            LogPrintf("Privacy: Failed to connect key images for tx %s\n", tx.GetHash().ToString());
                        }
                    }
                }
            }
...
{
    // Get block height for X25X activation check
    int nHeight = 0;
    {
        LOCK(cs_main);
        auto it = chainstate.m_blockman.m_block_index.find(block.hashPrevBlock);
        if (it != chainstate.m_blockman.m_block_index.end()) {
            nHeight = it->second.nHeight + 1;
        } else if (block.GetHash() == consensusParams.hashGenesisBlock) {
            nHeight = 0;  // Genesis block
        }
        // If prev not found and not genesis, height stays 0 (pre-X25X behavior)
    }

    // Check proof of work matches claimed amount
    if (fCheckPOW && block.IsProofOfWork() && !CheckHeaderPoWAtHeight(block, nHeight, consensusParams))
...
bool CheckBlock(const CBlock& block, BlockValidationState& state, const Consensus::Params& consensusParams, Chainstate& chainstate, bool fCheckPOW, bool fCheckMerkleRoot, bool fCheckSig)
{
    ...
    // Handle AuxPoW (merged-mined) blocks
    bool fAuxPowChecked = false;
    if (fCheckPOW && block.HasAuxPowFlag()) {
        // Block claims to be merge-mined
        if (!block.auxpow) {
            // AuxPoW flag set but no proof attached - invalid
            return state.Invalid(BlockValidationResult::BLOCK_INVALID_HEADER, "auxpow-missing",
                                 "AuxPoW flag set but no proof provided");
        }

        // Get block height for activation check
        int nHeight = 0;
        {
            LOCK(cs_main);
            auto it = chainstate.m_blockman.m_block_index.find(block.hashPrevBlock);
        ...
        // Check if AuxPoW is active at this height
        if (!consensus::IsAuxPowActive(nHeight, consensusParams)) {
            return state.Invalid(BlockValidationResult::BLOCK_INVALID_HEADER, "auxpow-not-active",
                                 "AuxPoW not active at this height");
        }

        // Validate the AuxPoW proof
        if (!consensus::CheckAuxProofOfWork(block, *block.auxpow, block.nBits, consensusParams)) {
            return state.Invalid(BlockValidationResult::BLOCK_INVALID_HEADER, "auxpow-invalid",
                                 "AuxPoW proof validation failed");
        }

        fAuxPowChecked = true;
        LogPrintf("CheckBlock: AuxPoW validation passed for block %s\n",
                  block.GetHash().GetHex().substr(0, 16));
    }

    // Check that the header is valid (particularly PoW).  This is mostly
    // redundant with the call in AcceptBlockHeader.
    // Skip PoW check if we already validated AuxPoW above
    if (!CheckBlockHeader(block, state, consensusParams, chainstate, fCheckPOW && !fAuxPowChecked, false))
        return false;
...
bool Chainstate::UpdateHashProof(const CBlock& block, BlockValidationState& state, const Consensus::Params& consensusParams, CBlockIndex* pindex, CCoinsViewCache& view)
{
    ...
    // WATTx Hybrid PoW/PoS Consensus
    // After nLastPOWBlock, both PoW (X25X multi-algo) and PoS blocks are valid
    // Miners and stakers compete equally for block rewards with Proof of Trust
...
bool ChainstateManager::AcceptBlockHeader(const CBlockHeader& block, BlockValidationState& state, CBlockIndex** ppindex, bool min_pow_checked)
{
    ...
    if (hash != GetConsensus().hashGenesisBlock) {
        ...
        // WATTx Hybrid PoW/PoS + Proof of Trust Consensus
        // Both PoW (X25X multi-algorithm) and PoS blocks are accepted
        // Miners and stakers compete equally for block rewards
        int nHeight = pindexPrev->nHeight + 1;
...
bool ChainstateManager::AcceptBlock(const std::shared_ptr<const CBlock>& pblock, BlockValidationState& state, CBlockIndex** ppindex, bool fRequested, const FlatFilePos* dbp, bool* fNewBlock, bool min_pow_checked)
{
    ...
    // WATTx Hybrid PoW/PoS + Proof of Trust Consensus
    // Both PoW (X25X multi-algorithm) and PoS blocks are accepted
    // Miners and stakers compete equally for block rewards based on trust tier
