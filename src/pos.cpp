...
bool CheckProofOfStake(CBlockIndex* pindexPrev, BlockValidationState& state, const CTransaction& tx, unsigned int nBits, uint32_t nTimeBlock, const std::vector<unsigned char>& vchPoD,  const COutPoint& headerPrevout, uint256& hashProofOfStake, uint256& targetProofOfStake, CCoinsViewCache& view, Chainstate& chainstate)
{
    ...
    // WATTx: Check for double-spending vulnerability
    // Verify this UTXO isn't already used in a competing coinstake at this height
    int nHeight = pindexPrev->nHeight + 1;
    if (!GetCoinstakeTracker().IsUTXOAvailableForStaking(txin.prevout, nHeight)) {
        return state.Invalid(BlockValidationResult::BLOCK_INVALID_HEADER, "coinstake-utxo-reuse",
            strprintf("CheckProofOfStake() : UTXO %s already used in coinstake at height %d",
                      txin.prevout.ToString(), nHeight));
    }
    ...
    // nHeight already declared above for UTXO tracking
...
//////////////////////////////////////////////////
// WATTx Trust Tier PoS Functions Implementation
//////////////////////////////////////////////////

bool CheckMinimumValidatorStake(CAmount nStakeAmount, const Consensus::Params& params)
{
    return nStakeAmount >= params.nMinValidatorStake;
}

bool CheckValidatorTrustTier(const CKeyID& validatorId, trust::TrustScoreManager& trustManager, const Consensus::Params& params)
{
    // Get the validator's trust tier
    trust::TrustTier tier = trustManager.GetValidatorTier(validatorId);

    // Validator must have at least Bronze tier (95% uptime)
    return tier != trust::TrustTier::NONE;
}
...
{
    return nHeight >= params.nTrustTierActivationHeight;
}
...
{
    if (!tx.IsCoinStake()) {
        return state.Invalid(BlockValidationResult::BLOCK_CONSENSUS, "not-coinstake",
                            "CheckTieredProofOfStake(): called on non-coinstake");
    }
    ...
    if (!IsTrustTierActive(nHeight, params)) {
        // Trust tier not active yet, skip additional checks
        return true;
    }
    ...
    Coin coinPrev;
    if (!ViewGetCoin(view, txin.prevout, coinPrev)) {
        return state.Invalid(BlockValidationResult::BLOCK_CONSENSUS, "stake-coin-not-found",
                            "CheckTieredProofOfStake(): Could not find stake coin");
    }

    // Check minimum stake requirement (100,000 WATTx)
    if (!CheckMinimumValidatorStake(coinPrev.out.nValue, params)) {
        return state.Invalid(BlockValidationResult::BLOCK_CONSENSUS, "stake-below-minimum",
                            strprintf("CheckTieredProofOfStake(): Stake amount %lld below minimum %lld",
                                      coinPrev.out.nValue, params.nMinValidatorStake));
    }

    // Extract validator key ID from the stake output
    CTxDestination dest;
    if (!ExtractDestination(coinPrev.out.scriptPubKey, dest)) {
        return state.Invalid(BlockValidationResult::BLOCK_CONSENSUS, "stake-invalid-script",
                            "CheckTieredProofOfStake(): Could not extract destination from stake");
    }

    if (!std::holds_alternative<PKHash>(dest)) {
        return state.Invalid(BlockValidationResult::BLOCK_CONSENSUS, "stake-not-p2pkh",
                            "CheckTieredProofOfStake(): Stake must be P2PKH");
    }

    CKeyID validatorId = ToKeyID(std::get<PKHash>(dest));

    // Check validator's trust tier
    trust::TrustTier tier = trustManager.GetValidatorTier(validatorId);
    if (tier == trust::TrustTier::NONE) {
        // Check if validator is registered but just doesn't meet uptime requirements
        const trust::ValidatorInfo* info = trustManager.GetValidator(validatorId);
        if (info == nullptr) {
            return state.Invalid(BlockValidationResult::BLOCK_CONSENSUS, "validator-not-registered",
                                "CheckTieredProofOfStake(): Validator is not registered");
        }

        // Validator exists but doesn't meet uptime requirement
        return state.Invalid(BlockValidationResult::BLOCK_CONSENSUS, "validator-low-uptime",
                            strprintf("CheckTieredProofOfStake(): Validator uptime %d%% below minimum 95%%",
