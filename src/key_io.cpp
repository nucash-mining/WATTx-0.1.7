...
public:
    ...
    std::string operator()(const DilithiumPubKeyHash& id) const
    {
        std::vector<unsigned char> data = m_params.Base58Prefix(CChainParams::DILITHIUM_ADDRESS);
        data.insert(data.end(), id.begin(), id.end());
        return EncodeBase58Check(data);
    }
};
...
CTxDestination DecodeDestination(const std::string& str, const CChainParams& params, std::string& error_str, std::vector<int>* error_locations)
{
    ...
    if (!is_bech32 && DecodeBase58Check(str, data, 21)) {
        ...
        // Dilithium (quantum-resistant) addresses
        // The data vector contains RIPEMD160(SHA256(dilithium_pubkey))
        const std::vector<unsigned char>& dilithium_prefix = params.Base58Prefix(CChainParams::DILITHIUM_ADDRESS);
        if (data.size() == hash.size() + dilithium_prefix.size() && std::equal(dilithium_prefix.begin(), dilithium_prefix.end(), data.begin())) {
            ...
            return DilithiumPubKeyHash(hash);
        }
