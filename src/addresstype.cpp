...
// Dilithium public key hash - HASH160 of the full Dilithium public key
DilithiumPubKeyHash::DilithiumPubKeyHash(const std::vector<unsigned char>& pubkey)
    : BaseHash(Hash160(pubkey)) {}
...
public:
    ...
    // Dilithium address script: OP_DUP OP_HASH160 <hash> OP_EQUALVERIFY OP_CHECKSIG_DILITHIUM
    CScript operator()(const DilithiumPubKeyHash& keyID) const
    {
        return CScript() << OP_DUP << OP_HASH160 << ToByteVector(keyID) << OP_EQUALVERIFY << OP_CHECKSIG_DILITHIUM;
    }
};
...
public:
    ...
    bool operator()(const DilithiumPubKeyHash& dest) const { return true; }
};
} // namespace
...
valtype DataVisitor::operator()(const DilithiumPubKeyHash& dilithiumKeyHash) const { return valtype(dilithiumKeyHash.begin(), dilithiumKeyHash.end()); }
