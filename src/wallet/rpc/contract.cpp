        ...
        [&,blockGasLimit,minGasPrice,nGasPrice](const RPCHelpMan& self, const JSONRPCRequest& request) mutable -> UniValue
{
    ...
    result.pushKV("address", FormatEvmAddress(contractAddress));
    }else{
...
UniValue SendToContract(CWallet& wallet, const UniValue& params, ChainstateManager& chainman)
{
    ...
    if (!NormalizeEvmAddress(params[0].get_str(), contractaddress))
...
RPCHelpMan sendtocontract()
{
    ...
    return RPCHelpMan{"sendtocontract",
                    ...
                    HELP_REQUIRING_PASSPHRASE,
                    {
                        ...
                        {"gasprice", RPCArg::Type::AMOUNT, RPCArg::Optional::OMITTED, "gasPrice WATTx price per gas unit, default: "+FormatMoney(nGasPrice)+", min:"+FormatMoney(minGasPrice)},
