...
RPCHelpMan setstaking()
{
    return RPCHelpMan{"setstaking",
                "\nEnable or disable staking for the wallet.",
                {
                    {"enabled", RPCArg::Type::STR, RPCArg::Optional::NO, "true/1 to enable staking, false/0 to disable"},
                },
                RPCResult{
                    RPCResult::Type::BOOL, "", "The staking state after the call"
                },
                RPCExamples{
        ...
        [&](const RPCHelpMan& self, const JSONRPCRequest& request) -> UniValue
{
    ...
    if (request.params[0].isBool()) {
        enable = request.params[0].get_bool();
    } else {
        std::string str = request.params[0].get_str();
        if (str == "true" || str == "1") enable = true;
        else if (str == "false" || str == "0") enable = false;
        else throw JSONRPCError(RPC_INVALID_PARAMETER, "Invalid boolean value: " + str);
    }
    pwallet->m_enabled_staking = enable;

    return pwallet->m_enabled_staking.load();
},
    };
}
    ...
    return RPCHelpMan{"getstakinginfo",
                ...
                RPCResult{
                    RPCResult::Type::OBJ, "", "",
                    {
                        ...
                        {RPCResult::Type::STR_AMOUNT, "minvalidatorstake", "Minimum stake required for super staker"},
                        {RPCResult::Type::NUM, "stakematurity", "Current blocks required for coins to be stakeable (halves each reward halving)"},
                        {RPCResult::Type::NUM, "basestakematurity", "Base stake maturity at Era 0 (500 blocks)"},
                        {RPCResult::Type::NUM, "stakematurityera", "Current halving era (0=500, 1=250, 2=125, etc.)"},
                        {RPCResult::Type::STR, "errors", "Error messages"},
        ...
        [&](const RPCHelpMan& self, const JSONRPCRequest& request) -> UniValue
{
    ...
    int nHeight = chainman.m_best_header->nHeight;
    int stakeMaturity = consensusParams.StakeMinConfirmations(nHeight);
    int baseStakeMaturity = consensusParams.nStakeMinConfirmations;
    int halvingEra = (stakeMaturity == baseStakeMaturity) ? 0 :
                     static_cast<int>(std::log2(static_cast<double>(baseStakeMaturity) / stakeMaturity));
    ...
    obj.pushKV("stakematurity", stakeMaturity); // Current stake maturity (halves with each reward halving)
    obj.pushKV("basestakematurity", baseStakeMaturity); // Base stake maturity (Era 0)
    obj.pushKV("stakematurityera", halvingEra); // Current halving era for stake maturity
    obj.pushKV("errors", pwallet->chain().getWarnings().original);
