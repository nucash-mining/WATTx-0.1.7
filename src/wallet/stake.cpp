...
bool CreateCoinStakeFromMine(CWallet& wallet, unsigned int nBits, const CAmount& nTotalFees, uint32_t nTimeBlock, CMutableTransaction& tx, PKHash& pkhash, std::set<std::pair<const CWalletTx*,unsigned int> >& setCoins, std::vector<COutPoint>& setSelectedCoins, bool selectedOnly, bool sign, COutPoint& headerPrevout)
{
    ...
    // Calculate reward
    {
        ...
        // WATTx Hybrid PoW/PoS: After X25X activation, split reward between miner and validator
        ...
        {
            // Split the reward when mpos is used (legacy MPoS)
...
bool CreateCoinStakeFromDelegate(CWallet& wallet, unsigned int nBits, const CAmount& nTotalFees, uint32_t nTimeBlock, CMutableTransaction& tx, PKHash& pkhash, std::set<std::pair<const CWalletTx*,unsigned int> >& setCoins, std::vector<COutPoint>& setDelegateCoins, bool sign, std::vector<unsigned char>& vchPoD, COutPoint& headerPrevout)
{
    ...
    // Calculate reward
    {
        ...
        // WATTx Hybrid PoW/PoS: After X25X activation, split reward between miner and validator
        if (nNextHeight >= consensusParams.nX25XActivationHeight) {
            // Calculate validator's share (PoS portion) - miner gets the rest in block creation
            int64_t nValidatorShare = (nTotalReward * consensusParams.nPoSRewardPercent) / 100;

            // Split validator share between staker and delegator
            int64_t nRewardStaker = 0;
            if(!SplitOfflineStakeReward(nValidatorShare, delegation.fee, nRewardOffline, nRewardStaker)) {
                LogError("CreateCoinStake: Failed to split reward for hybrid consensus");
                return false;
            }
...
bool SelectCoinsForStaking(const CWallet& wallet, CAmount &nTargetValue, std::set<std::pair<const CWalletTx *, unsigned int> > &setCoinsRet, CAmount &nValueRet)
{
    ...
    // Use dynamic StakeMinConfirmations - halves with each reward halving
    // Era 0: 500, Era 1: 250, Era 2: 125, etc. (makes PoS easier as PoW gets harder)
    int stakeMinConfirmations = Params().GetConsensus().StakeMinConfirmations(nHeight);
