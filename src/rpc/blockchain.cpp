        ...
        [&](const RPCHelpMan& self, const JSONRPCRequest& request) -> UniValue
{
    ...
    std::string strAddr;
    if (!NormalizeEvmAddress(request.params[0].get_str(), strAddr))
        throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Incorrect address (expected 40 hex chars, with optional 0x prefix)");
    ...
    result.pushKV("address", FormatEvmAddress(strAddr));
