...
static bool GenerateBlock(ChainstateManager& chainman, CBlock&& block, uint64_t& max_tries, std::shared_ptr<const CBlock>& block_out, bool process_new_block)
{
    ...
    // Get current chain height to determine if X25X is active
    int nHeight = 0;
    {
        LOCK(cs_main);
        nHeight = chainman.ActiveChain().Height() + 1;  // Height of block being mined
    }

    // Check if X25X is active at this height
    bool useX25X = params.IsX25XActive(nHeight);

    if (useX25X) {
        // X25X mining: get algorithm from block version (set by caller, or default to SHA256D)
        x25x::Algorithm algo = x25x::GetBlockAlgorithm(block.nVersion);

        // If no algorithm was set (version has default), use SHA256D
        if (algo == x25x::Algorithm::SHA256D && (block.nVersion & 0x0000FF00) == 0) {
            block.nVersion = x25x::SetBlockAlgorithm(block.nVersion, algo);
        }

        while (max_tries > 0 && block.nNonce < std::numeric_limits<uint32_t>::max() && !chainman.m_interrupt) {
            // Use X25X hash function for the specified algorithm
            uint256 hash = x25x::HashBlockHeader(block, algo);
        ...
        // Pre-X25X: use RandomX for proof-of-work mining
        while (max_tries > 0 && block.nNonce < std::numeric_limits<uint32_t>::max() && !chainman.m_interrupt) {
...
static RPCHelpMan generatetoaddress()
{
    return RPCHelpMan{"generatetoaddress",
        ...
        "Optionally specify the mining algorithm for X25X multi-algorithm testing.",
         {
             {"nblocks", RPCArg::Type::NUM, RPCArg::Optional::NO, "How many blocks are generated."},
             {"address", RPCArg::Type::STR, RPCArg::Optional::NO, "The address to send the newly generated qtum to."},
             {"maxtries", RPCArg::Type::NUM, RPCArg::Default{DEFAULT_MAX_TRIES}, "How many iterations to try."},
             {"algo", RPCArg::Type::STR, RPCArg::Optional::OMITTED, "Mining algorithm: sha256d, scrypt, ethash, randomx, equihash, x11, kheavyhash (default: sha256d)"},
         },
         RPCResult{
             RPCResult::Type::ARR, "", "hashes of blocks generated",
             {
         ...
         RPCExamples{
            "\nGenerate 11 blocks to myaddress using default algorithm\n"
            + HelpExampleCli("generatetoaddress", "11 \"myaddress\"")
            + "\nGenerate 5 blocks using RandomX algorithm\n"
            + HelpExampleCli("generatetoaddress", "5 \"myaddress\" 1000000 \"randomx\"")
            + "\nGenerate 5 blocks using Ethash algorithm\n"
            + HelpExampleCli("generatetoaddress", "5 \"myaddress\" 1000000 \"ethash\"")
        ...
        [&](const RPCHelpMan& self, const JSONRPCRequest& request) -> UniValue
{
    ...
    // Parse optional algorithm parameter
    std::optional<x25x::Algorithm> algo = std::nullopt;
    if (!request.params[3].isNull()) {
        std::string algoStr = request.params[3].get_str();
        // Convert to lowercase for comparison
        std::transform(algoStr.begin(), algoStr.end(), algoStr.begin(), ::tolower);

        if (algoStr == "sha256d" || algoStr == "sha256") {
            algo = x25x::Algorithm::SHA256D;
        } else if (algoStr == "scrypt") {
            algo = x25x::Algorithm::SCRYPT;
        } else if (algoStr == "ethash") {
            algo = x25x::Algorithm::ETHASH;
        } else if (algoStr == "randomx") {
            algo = x25x::Algorithm::RANDOMX;
        } else if (algoStr == "equihash" || algoStr == "zhash") {
            algo = x25x::Algorithm::EQUIHASH;
        } else if (algoStr == "x11") {
            algo = x25x::Algorithm::X11;
        } else if (algoStr == "kheavyhash" || algoStr == "kaspa") {
            algo = x25x::Algorithm::KHEAVYHASH;
        } else {
            throw JSONRPCError(RPC_INVALID_PARAMETER,
                strprintf("Unknown algorithm '%s'. Valid options: sha256d, scrypt, ethash, randomx, equihash, x11, kheavyhash", algoStr));
        }

        // Check if algorithm is enabled
        if (!x25x::IsAlgorithmEnabled(algo.value())) {
            throw JSONRPCError(RPC_INVALID_PARAMETER,
                strprintf("Algorithm '%s' is not enabled", algoStr));
        }
    }
    ...
    return generateBlocks(chainman, miner, coinbase_output_script, num_blocks, max_tries, algo);
},
    };
}
...
// X25X Multi-Algorithm Mining RPC Commands

static RPCHelpMan getx25xalgorithms()
{
    return RPCHelpMan{"getx25xalgorithms",
        "\nReturns list of available X25X mining algorithms.\n",
        {},
        RPCResult{
            RPCResult::Type::ARR, "", "",
            {
                {RPCResult::Type::OBJ, "", "",
                {
                    ...
                    {RPCResult::Type::BOOL, "enabled", "Whether algorithm is enabled"},
                    {RPCResult::Type::BOOL, "available", "Whether algorithm is available on this system"},
                    {RPCResult::Type::BOOL, "supports_merged_mining", "Whether algorithm supports merged mining"},
                }},
            }
        },
        RPCExamples{
            HelpExampleCli("getx25xalgorithms", "")
            + HelpExampleRpc("getx25xalgorithms", "")
        },
        [&](const RPCHelpMan& self, const JSONRPCRequest& request) -> UniValue
{
    ...
    for (const auto& algo : x25x::GetEnabledAlgorithms()) {
        ...
        UniValue obj(UniValue::VOBJ);
        obj.pushKV("name", info.name);
        obj.pushKV("description", info.description);
        obj.pushKV("enabled", info.enabled);
        obj.pushKV("available", node::X25XMiner::IsAlgorithmAvailable(algo));
        obj.pushKV("supports_merged_mining", info.supportsMergedMining);
...
static RPCHelpMan getminingalgorithm()
{
    return RPCHelpMan{"getminingalgorithm",
        "\nReturns the current mining algorithm.\n",
        {},
        RPCResult{
            RPCResult::Type::OBJ, "", "",
            {
                {RPCResult::Type::STR, "algorithm", "Current mining algorithm name"},
                {RPCResult::Type::NUM, "hashrate", "Current hashrate (H/s)"},
                {RPCResult::Type::BOOL, "mining", "Whether mining is active"},
            }
        },
        RPCExamples{
            HelpExampleCli("getminingalgorithm", "")
            + HelpExampleRpc("getminingalgorithm", "")
        },
        [&](const RPCHelpMan& self, const JSONRPCRequest& request) -> UniValue
{
    auto& miner = node::GetX25XMiner();
    const auto& info = x25x::GetAlgorithmInfo(miner.GetAlgorithm());

    UniValue result(UniValue::VOBJ);
    result.pushKV("algorithm", info.name);
...
static RPCHelpMan setminingalgorithm()
{
    return RPCHelpMan{"setminingalgorithm",
        "\nSets the mining algorithm to use.\n",
        {
            {"algorithm", RPCArg::Type::STR, RPCArg::Optional::NO, "Algorithm name (sha256d, scrypt, ethash, randomx, equihash, x11, kheavyhash/kaspa)"},
        },
        RPCResult{
            RPCResult::Type::OBJ, "", "",
            {
                {RPCResult::Type::BOOL, "success", "Whether algorithm was set successfully"},
                {RPCResult::Type::STR, "algorithm", "New mining algorithm"},
                {RPCResult::Type::STR, "message", "Status message"},
            }
        },
        RPCExamples{
            HelpExampleCli("setminingalgorithm", "\"randomx\"")
            + HelpExampleRpc("setminingalgorithm", "\"randomx\"")
        },
        [&](const RPCHelpMan& self, const JSONRPCRequest& request) -> UniValue
{
    std::string algoName = request.params[0].get_str();
    x25x::Algorithm algo = x25x::GetAlgorithmByName(algoName);

    if (algo == x25x::Algorithm::INVALID) {
        throw JSONRPCError(RPC_INVALID_PARAMETER, "Unknown algorithm: " + algoName);
    }

    if (!x25x::IsAlgorithmEnabled(algo)) {
        throw JSONRPCError(RPC_INVALID_PARAMETER, "Algorithm is not enabled: " + algoName);
    }

    auto& miner = node::GetX25XMiner();

    if (miner.IsMining()) {
        throw JSONRPCError(RPC_IN_WARMUP, "Cannot change algorithm while mining is active. Stop mining first.");
    }

    miner.SetAlgorithm(algo);

    const auto& info = x25x::GetAlgorithmInfo(algo);
    UniValue result(UniValue::VOBJ);
    result.pushKV("success", true);
    result.pushKV("algorithm", info.name);
    result.pushKV("message", "Mining algorithm set to " + info.name);
...
static RPCHelpMan getx25xmininginfo()
{
    return RPCHelpMan{"getx25xmininginfo",
        "\nReturns X25X multi-algorithm mining information.\n",
        {},
        RPCResult{
            RPCResult::Type::OBJ, "", "",
            {
                {RPCResult::Type::STR, "current_algorithm", "Current mining algorithm"},
                {RPCResult::Type::BOOL, "x25x_active", "Whether X25X is active on the network"},
                {RPCResult::Type::NUM, "x25x_activation_height", "X25X activation height"},
                {RPCResult::Type::NUM, "hashrate", "Current hashrate (H/s)"},
                {RPCResult::Type::NUM, "total_hashes", "Total hashes computed"},
                {RPCResult::Type::BOOL, "mining", "Whether mining is active"},
                {RPCResult::Type::ARR, "available_algorithms", "List of available algorithm names",
                {
        ...
        RPCExamples{
            HelpExampleCli("getx25xmininginfo", "")
            + HelpExampleRpc("getx25xmininginfo", "")
        },
        [&](const RPCHelpMan& self, const JSONRPCRequest& request) -> UniValue
{
    ChainstateManager& chainman = EnsureAnyChainman(request.context);
    ...
    UniValue result(UniValue::VOBJ);
    result.pushKV("current_algorithm", info.name);
    result.pushKV("x25x_active", params.IsX25XActive(currentHeight));
    result.pushKV("x25x_activation_height", params.nX25XActivationHeight);
    result.pushKV("hashrate", miner.GetHashrate());
    result.pushKV("total_hashes", (uint64_t)miner.GetTotalHashes());
    result.pushKV("mining", miner.IsMining());

    UniValue algos(UniValue::VARR);
    for (const auto& algo : node::X25XMiner::GetAvailableAlgorithms()) {
        algos.push_back(x25x::GetAlgorithmInfo(algo).name);
    }
    result.pushKV("available_algorithms", algos);
...
void RegisterMiningRPCCommands(CRPCTable& t)
{
    static const CRPCCommand commands[]{
        ...
        // X25X multi-algorithm mining commands
        {"mining", &getx25xalgorithms},
        {"mining", &getminingalgorithm},
        {"mining", &setminingalgorithm},
        {"mining", &getx25xmininginfo},
