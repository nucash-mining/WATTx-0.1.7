            ...
            [&](const RPCHelpMan& self, const JSONRPCRequest& request) -> UniValue
{
    ...
    return "0x" + ToKeyID(keyID).GetReverseHex();
},
    };
}
            ...
            [&](const RPCHelpMan& self, const JSONRPCRequest& request) -> UniValue
{
    ...
    std::string hexAddr;
    if (!NormalizeEvmAddress(request.params[0].get_str(), hexAddr))
        throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Invalid pubkeyhash hex size (should be 40 hex characters, with optional 0x prefix)");
    CKeyID id;
    ...
    if(isExpanded)
    {
        if(isVin) {
            return {
                ...
                {RPCResult::Type::STR, "address", /*optional=*/true, "The WATTx address (only if address index is enabled)"},
            };
        }
        else {
            return {
    ...
    void addContract(std::vector<std::pair<CTxDestination, CAmount>>& parsed_outputs, const UniValue& Contract) override
    {
        ...
        if (Contract.exists("senderAddress")){
            ...
            if (!IsValidDestination(senderAddress))
                throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Invalid WATTx address to send from");
        ...
        else
        {
            ...
            if (!NormalizeEvmAddress(Contract["contractAddress"].get_str(), contractaddress))
                throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Incorrect contract address (expected 40 hex chars, with optional 0x prefix)");
    ...
    return RPCHelpMan{
        ...
        RPCResult{
            RPCResult::Type::OBJ, "", "",
            {
                ...
                {RPCResult::Type::STR, "address", /*optional=*/true, "The WATTx address (only if a well-defined address exists)"},
...
static RPCHelpMan decodepsbt()
{
    return RPCHelpMan{
        "decodepsbt",
        "Return a JSON object representing the serialized, base64-encoded partially signed WATTx transaction.",
                {
...
static RPCHelpMan combinepsbt()
{
    return RPCHelpMan{"combinepsbt",
                "\nCombine multiple partially signed WATTx transactions into one transaction.\n"
