...
// Compatibility: C++20 fallbacks for older compilers (GCC < 11, MinGW)
#if __cpp_lib_bit_cast >= 201806L && __cpp_lib_endian >= 201907L && __cpp_lib_bitops >= 201907L
#include <bit>
#else
#include <climits>
namespace std {
...
[[nodiscard]] constexpr T rotl(T x, int s) noexcept {
    ...
    const auto r = s % static_cast<int>(N);
    if (r == 0) return x;
