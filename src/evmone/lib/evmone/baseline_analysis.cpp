...
CodeAnalysis analyze_legacy(bytes_view code)
{
    ...
    // C++17 compatible: use make_unique instead of make_unique_for_overwrite
    auto storage = std::unique_ptr<uint8_t[]>(new uint8_t[total_size]);
    std::copy(code.begin(), code.end(), storage.get());               // Copy code.
    std::fill_n(&storage[code.size()], total_size - code.size(), 0);  // Pad code and init bitset.

    auto* bitset_storage =
        reinterpret_cast<BitsetSpan::word_type*>(&storage[aligned_code_size]);
    BitsetSpan jumpdest_bitset{bitset_storage};
    analyze_jumpdests(jumpdest_bitset, code);

    return CodeAnalysis{std::move(storage), code.size(), jumpdest_bitset};
}
